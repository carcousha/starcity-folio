-- تطبيق جداول الإرسال الجماعي مباشرة
-- Direct Application of Bulk Messages Tables
...
CREATE TABLE IF NOT EXISTS whatsapp_bulk_messages (
    ...
    recipient_filters JSONB, -- {contact_types: [], companies: [], tags: [], exclude_numbers: []}
    custom_recipients JSONB, -- قائمة الأرقام المخصصة
    
    -- إعدادات الإرسال
    send_type VARCHAR(50) DEFAULT 'immediate' CHECK (send_type IN ('immediate', 'scheduled', 'gradual')),
    scheduled_at TIMESTAMP WITH TIME ZONE,
    gradual_settings JSONB, -- {batch_size: 10, delay_minutes: 5}
...
CREATE TABLE IF NOT EXISTS whatsapp_bulk_message_recipients (
    ...
    personalized_content TEXT, -- المحتوى المخصص للشخص
