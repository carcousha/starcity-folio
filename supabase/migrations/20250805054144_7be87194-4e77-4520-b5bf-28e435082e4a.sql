-- تبسيط نظام الصلاحيات وإضافة RLS policies المفقودة (مُعدّل)
...
DROP POLICY IF EXISTS "Admins and accountants can manage treasury" ON public.treasury_transactions;
...
DROP POLICY IF EXISTS "Admins and accountants can view treasury accounts" ON public.treasury_accounts;
...
DROP POLICY IF EXISTS "Admins can manage treasury accounts" ON public.treasury_accounts;
...
DROP POLICY IF EXISTS "Admins and accountants can view vehicles" ON public.vehicles;
...
DROP POLICY IF EXISTS "All authenticated users can view contracts" ON public.rental_contracts;
CREATE POLICY "All authenticated users can view contracts" 
ON public.rental_contracts 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Authenticated users can create contracts" ON public.rental_contracts;
CREATE POLICY "Authenticated users can create contracts" 
ON public.rental_contracts 
FOR INSERT 
WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Contract creators and admins can update" ON public.rental_contracts;
...
-- إنشاء صلاحيات افتراضية أساسية (حذف السابقة أولاً)
DELETE FROM public.permission_settings;
