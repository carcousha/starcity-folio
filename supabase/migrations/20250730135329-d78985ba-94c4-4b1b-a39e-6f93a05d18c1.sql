-- حل المشاكل الأمنية العاجلة
-- إزالة Security Definer Views المشكوك فيها

-- البحث عن Views بـ Security Definer وإصلاحها
-- أولاً نحدد الـ views الموجودة
SELECT schemaname, viewname, definition 
FROM pg_views 
WHERE schemaname IN ('public') 
AND definition ILIKE '%security definer%';

-- إذا وجدت views بـ Security Definer، سنعيد إنشاؤها بدون Security Definer
-- أو نحذفها إذا لم تكن ضرورية
...
WHERE 
  ...
  (
    -- المدراء يرون الجميع
    EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'admin'::app_role) OR
    -- المحاسبون يرون الموظفين
    (EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'accountant'::app_role)) OR
...
SELECT public.log_financial_activity(
  'security_update',
  'تم تحديث الإعدادات الأمنية وإزالة Security Definer Views',
  0,
  'system',
  gen_random_uuid(),
  NULL,
