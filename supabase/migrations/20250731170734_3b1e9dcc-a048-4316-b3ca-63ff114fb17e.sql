-- إنشاء bucket لقوالب PDF
INSERT INTO storage.buckets (id, name, public)
VALUES ('contract-templates', 'contract-templates', true);

-- سياسات التخزين للقوالب
CREATE POLICY "Admins can upload PDF templates"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'contract-templates' AND (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'accountant'::app_role)));

CREATE POLICY "Everyone can view PDF templates"
ON storage.objects FOR SELECT
USING (bucket_id = 'contract-templates');

CREATE POLICY "Admins can delete PDF templates"
ON storage.objects FOR DELETE
USING (bucket_id = 'contract-templates' AND (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'accountant'::app_role)));
