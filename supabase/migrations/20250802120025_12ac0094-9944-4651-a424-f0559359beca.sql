...
CREATE POLICY "Admin can manage government service workflow"
ON public.government_service_workflow FOR INSERT, UPDATE, DELETE
USING (public.is_admin())
WITH CHECK (public.is_admin());
...
ON public.notification_logs FOR INSERT, UPDATE, DELETE
USING (public.is_admin())
WITH CHECK (public.is_admin());
...
CREATE POLICY "Admin can manage PDF templates"
ON public.pdf_templates FOR INSERT, UPDATE, DELETE
USING (public.is_admin())
WITH CHECK (public.is_admin());
...
-- 6. Create comprehensive audit logging trigger
...
CREATE TRIGGER audit_revenues_changes AFTER INSERT OR UPDATE OR DELETE ON public.revenues
  FOR EACH ROW EXECUTE FUNCTION public.audit_table_changes();

CREATE TRIGGER audit_expenses_changes AFTER INSERT OR UPDATE OR DELETE ON public.expenses
  FOR EACH ROW EXECUTE FUNCTION public.audit_table_changes();

CREATE TRIGGER audit_commissions_changes AFTER INSERT OR UPDATE OR DELETE ON public.commissions
  FOR EACH ROW EXECUTE FUNCTION public.audit_table_changes();

CREATE TRIGGER audit_debts_changes AFTER INSERT OR UPDATE OR DELETE ON public.debts
  FOR EACH ROW EXECUTE FUNCTION public.audit_table_changes();