-- Fix the operation_type constraint to include all existing types
ALTER TABLE public.activity_logs DROP CONSTRAINT IF EXISTS activity_logs_operation_type_check;

-- Add updated constraint that includes all existing operation types
...
CHECK (operation_type IN (
  ...
  'expense_added', 'commission_processed', 'commission_created', 'vehicle_expense'
));