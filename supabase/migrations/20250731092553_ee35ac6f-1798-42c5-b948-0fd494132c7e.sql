-- إصلاح نظام إدارة المديونيات مع تصحيح الأخطاء
...
ADD COLUMN IF NOT EXISTS installment_frequency TEXT DEFAULT 'monthly' CHECK (installment_frequency IN ('weekly', 'monthly', 'quarterly', 'yearly')),
ADD COLUMN IF NOT EXISTS auto_deduct_from_commission BOOLEAN DEFAULT false,
...
-- دالة لإنشاء الأقساط التلقائية (مع إصلاح مشكلة current_date)
...
BEGIN
  ...
  FOR i IN 1..p_installment_count LOOP
    ...
    next_due_date := CASE p_frequency
...
BEGIN
  -- جدولة تنبيهات الأقساط المستحقة خلال 7 أيام
  ...
  -- جدولة تنبيهات الأقساط المتأخرة
  FOR installment_record IN 
